...
hitrate <- read.csv("hit-rate.csv")
slowdown <- read.csv("slowdown.csv")
dmutil <- read.csv("dm-util.csv")
...
ggsave("hit-rate.pdf",width=9, height=5.5)

slowdown$co.runners <- factor(slowdown$co.runners,as.character(slowdown$co.runners))
slowdown$Benchmark <- factor(slowdown$Benchmark,as.character(slowdown$Benchmark))
ggplot(slowdown,aes(Benchmark,y=slowdown,fill=co.runners)) + geom_bar(width=0.6,stat="identity",position="dodge",color="black") + theme_bw()+scale_fill_grey(start = 0, end = .9)+theme(axis.text.x=element_text(angle=-30,hjust=0,size=25),axis.title.x = element_blank(),axis.text.y=element_text(size=25),axis.title.y=element_text(size=25),legend.position="top",legend.direction="horizontal",legend.title=element_blank(),legend.text=element_text(size=27))+ ylab("Slowdown")+ coord_cartesian(ylim = c(0, 2)) + geom_abline(intercept = 1, slope = 0,  linetype="dotted")
ggsave("slowdown.pdf",width=9, height=5.5)
...
ggsave("dm-util.pdf",width=9, height=5.5)
